...
// getFieldId: obtiene el fieldId (fldXXX) de un campo por nombre desde el schema
async function getFieldId(fieldName) {
  try {
    ...
    if (!table) return null;
    const norm = function(s){ return s.toLowerCase().replace(/[áàäéèëíìïóòöúùü]/g, function(c){
      return {á:'a',à:'a',ä:'a',é:'e',è:'e',ë:'e',í:'i',ì:'i',ï:'i',ó:'o',ò:'o',ö:'o',ú:'u',ù:'u',ü:'u'}[c]||c;
    }).trim(); };
    const field = (table.fields || []).find(function(f){ return norm(f.name) === norm(fieldName); });
    if (!field) { console.warn('[UPLOAD] campo no encontrado en schema:', fieldName); return null; }
    console.log('[UPLOAD] fieldId=' + field.id + ' para "' + fieldName + '"');
    return field.id;
  } catch(e) { console.warn('[UPLOAD] getFieldId error:', e.message); return null; }
}

// uploadFileToField: sube un único PDF a un campo adjunto via Content API
async function uploadFileToField(recordId, fieldName, file) {
  ...
  let b64 = String(file.base64 || '');
  const c = b64.indexOf(','); if (c !== -1) b64 = b64.slice(c + 1);
  const fname = file.filename || file.name || 'archivo.pdf';
  const ctype = file.contentType || file.type || 'application/pdf';
  console.log('[UPLOAD] subiendo "' + fname + '" → campo "' + fieldName + '" b64len=' + b64.length);
...
// uploadCertificates: sube N certificados PDF al campo de calibración
async function uploadCertificates(recordId, fieldName, files) {
  console.log('[CERT] record=' + recordId + ' n=' + (files||[]).length);
  ...
  for (let i = 0; i < valid.length; i++) {
    ...
    else errors.push({ filename: valid[i].filename || valid[i].name, status: r.status, error: r.error });
  }
